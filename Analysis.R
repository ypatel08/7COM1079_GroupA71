...
if ("Sales.Quantity" %in% names(dept_data)) {
  cor_test <- cor.test(dept_data$Revenue, dept_data$Sales.Quantity, method = "pearson")
  capture.output(cor_test, file = "correlation_summary.txt")
  
  # Scatterplot
  ggplot(dept_data, aes(x = Sales.Quantity, y = Revenue)) +
  ...
  ggsave("scatter_revenue_vs_quantity.png", width = 6, height = 4, dpi = 300)
} else {
  write("Column 'Sales.Quantity' not found in dataset", file = "correlation_summary.txt")
}